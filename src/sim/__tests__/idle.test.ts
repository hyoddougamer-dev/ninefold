  ...
  it('leaves the lightest cultivator standing at a ceiling for most of the first realm', () => {
    ...
    for (const run of runs) {
      const rows = run.rows.map((r) =>
        ...
        `   ${r.ceilingHours.toFixed(0).padStart(4)}h at the ceiling` +
        ` (${(r.ceilingHours / r.hours * 100).toFixed(0).padStart(3)}% of the realm,` +
        ` ${(r.ceilingQi * 100).toFixed(0).padStart(3)}% of its qi)`);
    ...
    /**
     * 頂 The finding. A realm's bar stops at its ninth rung and qi banks until the warden
     * falls, and for somebody who opens the app once a day that is three quarters of the
     * first realm — the one realm where a player has not yet decided to stay.
     */
    expect(first(light).ceilingHours / first(light).hours).toBeGreaterThan(0.5);
    // And it is a *habit* problem rather than a pricing one: showing up shortens it.
    expect(first(busy).ceilingHours / first(busy).hours)
      .toBeLessThan(first(light).ceilingHours / first(light).hours);
    ...
    // By the middle realms the ceiling is a rounding error for everybody.
    for (const run of runs) {
      for (const row of run.rows.filter((r) => r.realm >= 5)) {
        expect(row.ceilingHours / row.hours).toBeLessThan(0.15);
      }
    }
  }, 120_000);
});

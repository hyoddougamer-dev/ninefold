...
export function salvageValue(item: Item): number {
  ...
  return Math.max(1, Math.round(rung * salvageShare(realm) * RARITY_INFO[item.rarity].mult));
}
...
export function salvageWorth(items: readonly Item[]): number {
  return items.reduce((sum, x) => sum + salvageValue(x), 0);
}

...
\u5224 the one line
  ...
  <section class="sec" id="combat">
    ...
    <div class="rule"><b>初 And twelve minutes was still too long.</b> Bruno, having watched
      ...
      <b>3.5 hours</b>. 引 The guide was tried opening on that fight and walked in the real
      game, and it stuck. The rat's first sight pays about 1,500 qi, which on top of the 800
      in hand opens a layer by itself. The player then arrived at "spend your qi" holding
      180 with nothing to afford. So the purse is spent first and the fight is the step right
      after, won from the first tap. The frog was left where it was on purpose: the
  ...
  <section class="sec" id="ranked">
    ...
    <div class="shots"><figure>
      ...
      <figcaption><b>Join, the board, a new device.</b> A name and a guest, or an email and
        the link it carries. On a new device the cloud's cultivator is offered before
        anything is synced.</figcaption>
    </figure></div>

    <h3>活 Live, and attacked on every deploy</h3>
    <p class="t">The server went live on 26 September 2026. Every deploy ends by attacking it
      with nothing but the public key a phone has: a guest signs up, syncs, lands on the
      board and names itself. It then fails to write a standing directly and fails to read
      anybody's save. A save with a week of qi in thirty seconds is not ranked, and a second
      sync straight after is refused. Last, the test player deletes itself and the board is
      checked for leftovers.</p>
    <div class="rows">
      <div class="row"><span class="body"><b class="cjk">鑰</b> <em>The old keys are off</em>
        <i>A secret key was pasted into a chat on the way here, so the legacy JWT keys were
        switched off the same day. The game uses the publishable key and the function uses
        the new secret key, and the attack passed again with the old ones gone.</i></span></div>
      <div class="row"><span class="body"><b class="cjk">信</b> <em>No code in the email, yet</em>
        <i>Supabase refuses custom email templates on the free plan without a mail server of
        its own. So the sign-in email carries the link only, and the screen no longer
        promises a code. Inside the APK a link opens the browser rather than the game, so
        testers there play as guests until a mail server is set up.</i></span></div>
      <div class="row"><span class="body"><b class="cjk">待</b> <em>A 200 is not the setting</em>
        <i>The first live run switched guests on and attacked two seconds later, and was told
  ...
  <section class="sec" id="testers">
    <h2><span class="h">測</span> What the first testers said, and what changed</h2>
    <p class="t">Bruno, after the first people played it: <i>"está muito difícil para os players
      começarem, não conseguem fazer nada até terem power suficiente para os primeiros
      monstros. É preciso ser mais intuitivo, e os rankings devem aparecer como destaque e não
      escondido no menu."</i> Three things, and each one is below as the real game draws it.</p>

    <h3>初 A fight from the first minute</h3>
    <p class="t">The rat stood at 力 2.4 and a new cultivator at 力 1, so the one button that
      is not a shop said no for twelve minutes. It stands at 力 0.7 now, won at 98% from the
      first tap, and the hound follows at about three quarters of an hour. The guide still
      spends the 800 qi first. Tried the other way round, the rat's first sight paid about
      1,500 qi and opened a layer by itself, so the player reached "spend your qi" with
      nothing to afford. And the hunt screen no longer opens on three zeroes and two ×1.00:
      the record appears with the first kill, so the rat is the first thing on the page.</p>
    <div class="shots"><figure>
      <img src="bible-art/shot/start.webp" alt="A new cultivator: the guide asking for the first purchase, the hunt screen opening on the rat at 98 percent, and the next step asking for material">
      <figcaption><b>The first minutes, at 400 wide.</b> Spend the purse, beat the rat at 98%,
        then the guide sends the player back to the hunt for 材 material.</figcaption>
    </figure></div>

    <h3>榜 The rankings are a tab</h3>
    <p class="t">They were one line in the corner menu, under the save and the help. Now they
      are the sixth tab, in gold so they read as a place to go, and once a player is on the
      Heaven List the tab carries their place, the way 道 carries its unspent points. On the
      PC they sit under their own rule at the foot of the rail. Six tabs fit a 320 phone: the
      labels give up a little spacing there and nothing else.</p>
    <div class="shots"><figure>
  ...
  <section class="sec" id="words">
    <h2><span class="h">判</span> The gear screen says it in words</h2>
    <p class="t">Bruno, on the gear audit: <i>"sinto que tem muito texto matemático que pode
      confundir players"</i>. Eight proposals went to him as a working mockup, and he chose
      the first three. Every picture below is the compiled game with the same realm 6
      cultivator, before and after.</p>

    <h3>總 Two numbers that are true</h3>
    <p class="t">The top of the screen was seven chips adding the lines up. The 氣 one read
      +333.9% on a body whose gear lifted the qi rate by a fifth, because the sum is taken
      before the ceiling bends it. Now it is <code>gearLift()</code>: the body against itself
      with nothing on, from <code>power()</code> and <code>rate()</code>. ×5.1 power and ×1.23
      qi, a sentence each, and the rest folded under "Other effects".</p>
    <div class="shots"><figure>
      <img src="bible-art/shot/gear-top.webp" alt="The top of the gear screen before, seven percentage chips, and after, two large multipliers with a sentence each">
      <figcaption><b>Before and after, 400 wide.</b> The ring is unchanged, and has its
        portrait back: it had gone blank when the aura became layers.</figcaption>
    </figure></div>

    <h3>判 The sheet opens on the answer</h3>
    <p class="t">A piece now opens on one of four words from <code>verdictOf()</code>: an
      upgrade, a trade, the same, or weaker. Under it, power and qi each move by a word
      from <code>sizeOf()</code> (a little, clearly, a lot, hugely), with the percentage small
      beside it. The five rows of lines are folded under "Every effect, in detail", and the
      qi row there says why it reads bigger than it is. A worn piece reads against the same
      place left empty, so it says what it is doing for you.</p>
    <div class="shots"><figure>
      <img src="bible-art/shot/gear-sheet.webp" alt="The item sheet before, five rows of percentages, and after, an upgrade and a weaker piece each opening on a word">
      <figcaption><b>The Starfall Horned Helm before and after, and a weaker fan.</b> A
        piece that is worse offers "Wear it anyway", in outline rather than in jade.</figcaption>
    </figure></div>

    <h3>▲ The chest leads with what is better</h3>
    <p class="t">Tiles carry no number now. A piece better than what is worn in its place
      has a jade ▲ on its corner. The upgrades come first, the biggest first, then the rarest.
      A row of filters narrows it to one place on the body or to the upgrades alone.</p>
    <div class="shots"><figure>
      <img src="bible-art/shot/gear-chest.webp" alt="The chest filtered to the four upgrades, and the whole chest sorted with the upgrades first">
      <figcaption><b>"▲ Better" and "All".</b> Four of twenty-four pieces are upgrades, and
        they are the first row.</figcaption>
    </figure></div>
  </section>
